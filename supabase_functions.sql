-- =====================================================
-- دوال SQL لإنشاء الجداول في Supabase
-- =====================================================

-- دالة إنشاء جدول السائقين
...
-- دالة إنشاء جدول إشعارات المتاجر
CREATE OR REPLACE FUNCTION create_store_notifications_table()
RETURNS void AS $$
BEGIN
...
-- دالة إنشاء جدول رسائل الدعم الفني
CREATE OR REPLACE FUNCTION create_support_messages_table()
RETURNS void AS $$
BEGIN
...
-- دالة إنشاء جدول رسائل دعم المتاجر
CREATE OR REPLACE FUNCTION create_store_support_messages_table()
RETURNS void AS $$
BEGIN
...
-- دالة إنشاء جدول الغرامات
CREATE OR REPLACE FUNCTION create_fines_table()
RETURNS void AS $$
BEGIN
...
-- دالة إنشاء جدول المستخدمين المحظورين
...
BEGIN
    ...
    -- إدخال غرامات تجريبية
    INSERT INTO fines (driver_id, amount, reason, status) VALUES
    (1, 20.00, 'تأخير في التوصيل', 'paid'),
    (2, 15.00, 'عدم ارتداء الزي الرسمي', 'active'),
    (3, 30.00, 'إلغاء طلب بدون سبب', 'active')
    ON CONFLICT DO NOTHING;

    -- إدخال طلبات تسجيل تجريبية
    INSERT INTO registration_requests (email, password, name, phone, user_type, status) VALUES
    ('newdriver@tawseel.com', 'password123', 'عبدالله أحمد', '+966501111111', 'driver', 'pending'),
    ('newstore@tawseel.com', 'password123', 'مطعم الجديد', '+966502222222', 'store', 'pending'),
    ('driver4@tawseel.com', 'password123', 'حسن محمد', '+966503333333', 'driver', 'approved')
    ON CONFLICT (email) DO NOTHING;
END;
$$ LANGUAGE plpgsql;
